...
-- AlterTable
ALTER TABLE `items` ADD COLUMN `price` DOUBLE NOT NULL DEFAULT 0.0;
...
-- AddForeignKey
ALTER TABLE `orders` ADD CONSTRAINT `orders_job_type_id_fkey` FOREIGN KEY (`job_type_id`) REFERENCES `job_types`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `item_order` ADD CONSTRAINT `item_order_item_id_fkey` FOREIGN KEY (`item_id`) REFERENCES `items`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
